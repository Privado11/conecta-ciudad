import type { CustomValidation, ValidationRule } from "@/shared/types/dynamicForm.types";
...
export function validateCustomRule(
  ...
  if (validation.validator) {
    ...
    return {
      isValid,
      message: isValid ? undefined : (validation.message || VALIDATION_MESSAGES.custom)
    };
  }
  ...
  if (pattern) {
    ...
    return {
      isValid,
      message: isValid ? undefined : (validation.message || VALIDATION_MESSAGES[validation.rule])
    };
  }
